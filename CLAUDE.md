...
- **`NMEAParser`**（QObject）— 带缓冲的 NMEA 0183 语句解析器。`feed(QByteArray)` 接收原始字节，按 `\n` 分割，去除 `$` 前缀，验证 XOR 校验和，分派到各语句解析函数。发送 `positionUpdated(GpsData)` 和 `rawSentence(QString)` 信号。支持的语句类型：`GGA`（定位/精度/卫星数）、`RMC`（时间/日期/速度/航向）、`VTG`（速度向量）、`GSA`（DOP 值）、`GSV`（可见卫星）。
- **`GpsData`**（结构体）— 包含：经纬度/海拔、速度（节+km/h）、航向角、定位质量（0=无效, 1=GPS, 2=DGPS, 4=RTK固定解, 5=RTK浮点解）、卫星数、HDOP/PDOP/VDOP、磁偏角、isValid 标志、`QList<SatelliteInfo>`（prn/仰角/方位角/信噪比）。
...
- **`GpsManager`** — 中央协调器。通过 `DriverFactory` 创建驱动，将驱动的 `rawDataReady` 连接到 `NMEAParser::feed()`。暴露 Q_PROPERTY：lat/lon/alt/speed/course/fixQuality/satelliteCount/hdop。QML 调用 `connectToSource(address, type, baudRate)` 建立连接。发送 `newPosition(GpsData)` 信号，供 TrackRecorder 和 GeofenceManager 消费。
...
- **`SatellitePanel.qml`** — `Canvas` 绘制的天空图（同心仰角环+方位角）和 SNR 柱状图。卫星列表表格。
  ...
  → GpsManager.onRawDataReady(bytes)
    → NMEAParser.feed(bytes)
      → NMEAParser::parseSentence() 逐行解析
        → validateChecksum()
        → parseGPGGA / parseGPRMC / parseGPVTG / parseGPGSA / parseGPGSV
      → 发送 positionUpdated(GpsData) 信号
    → GpsManager.onPositionUpdated(data)
      → 发送 positionChanged()             （QML 属性更新）
      → 发送 newPosition(data)             （如果数据有效）
        → TrackRecorder.addPoint(data)     （如果正在录制）
        → GeofenceManager.checkPosition()  （围栏进出检测）
```
